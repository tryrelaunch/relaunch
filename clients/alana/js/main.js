/* Alana Yacht Rental — main.js */

// ── BOOKING MODAL ──
const FAREHARBOR_URL = 'https://fareharbor.com/embeds/book/alanayachtrental/?full-items=yes';

function openBookModal() {
  const modal = document.getElementById('book-modal');
  const iframe = document.getElementById('book-modal-iframe');
  if (!modal || !iframe) return;
  iframe.src = FAREHARBOR_URL;
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && document.getElementById('book-modal')?.classList.contains('open')) {
    closeBookModal();
  }
});
